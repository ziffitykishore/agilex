 ...
 *
 * Class Url
 * @package RocketWeb\ShoppingFeeds\Model\Product\Mapper\Generic\Configurable\Associated;
 */
class Url extends MapperAbstract
{
    const XML_PATH_MICRODATA_ENABLED = 'shoppingfeeds/google/microdata_enabled';
    ...
    public function map(array $params = array())
    {
        ...
        if ((is_array($uniqueParams))) {
            ...
            if ($microdataEnabled) {
                $urlQuery .= empty($urlQuery) ? 'aid=' . $this->getAdapter()->getProduct()->getId() : '&aid=' . $this->getAdapter()->getProduct()->getId();
            }
            $urlQuery .= count($uniqueParams) > 0 ? '#' . http_build_query($uniqueParams) : '';
        }
